...
export function AtlasPage() {
  ...
  const catalog = useCatalog({ q: search, cuisine, max_minutes: minutes, status, photos, page, limit: 24 });
    ...
    {tab === "recipes" ? <>
      ...
      <div className="grid grid-cols-2 gap-2">
        <select value={cuisine} onChange={(e) => update(() => setCuisine(e.target.value))} aria-label="地区与菜系" className="min-w-0 rounded-xl bg-white p-2.5 text-xs">
          <option value="">全部地区与菜系</option>
          {nodes.filter(n => n.level === 3 && n.dish_count > 0).map(country => <optgroup key={country.id} label={country.name}>
            <option value={country.id}>{country.name} · 全部</option>
      ...
      <p className="mt-5 text-[11px] leading-5 text-neutral-400">实拍与社区配方来自 HowToCook；每道菜详情可查看原始出处与配方份量。尚未补图的菜会明确标注。</p>
